...
all_means=[]
for i in range(0,bre.shape[0]): #n_rows
    rowvals=[];term=bre.iloc[i][1]
    for x in bre.iloc[i][2:7]: #2:7 skips row nums, metric names
        rowvals.append(x)
    for x in cm.iloc[i][2:7]: #2:7 skips row nums, metric names
        rowvals.append(x)
    mean=np.mean(rowvals);std=np.std(rowvals)
    all_means.append((term,mean,std))

all_means.sort(key=lambda means : means[1]) #sort by mean
all_means.reverse() #reverse so highest means first
meandf=make_df(all_means)
meandf.to_csv('brecmmeans.csv')
def make_df(list):
    df=pd.DataFrame()
    names=[]
    means=[]
...
def f(metric):
    rowvals=[]
    for i in range(0,fd.shape[0]): #n_rows
        term=fd.iloc[i][1]
        if term==metric:
            for x in fd.iloc[i][2:7]: #2:7 skips row nums, metric names
                rowvals.append(x)
            for x in bre.iloc[i][2:7]: #2:7 skips row nums, metric names
                rowvals.append(x)
            for x in cm.iloc[i][2:7]: #2:7 skips row nums, metric names
...
exphyd=f('exphyd')
fa_elec=f('fa_elec')
hbonds_int=f('hbonds_int')
hbond_bb_sc=f('hbond_bb_sc')
nres_int=f('nres_int')
hbond_lr_bb=f('hbond_lr_bb')
fa_atr=f('fa_atr')
lk_ball_wtd=f('lk_ball_wtd')
cav=f('cav')
buns_bb_heavy=f('buns_bb_heavy')
hbond_sr_bb=f('hbond_sr_bb')
acc=f('acc')
p_aa_pp=f('p_aa_pp')
packstat=f('packstat')
dSASA_int=f('dSASA_int')
...
y2=[]
y3=[]
for i in hbond_sc[0:5]:
    y1.append(i)
for i in ref[0:5]:
    y1.append(i)
for i in exphyd[0:5]:
    y1.append(i)
for i in fa_elec[0:5]:
    y1.append(i)
for i in hbonds_int[0:5]:
    y1.append(i)
for i in hbond_bb_sc[0:5]:
    y1.append(i)
for i in nres_int[0:5]:
    y1.append(i)
for i in hbond_lr_bb[0:5]:
    y1.append(i)
for i in fa_atr[0:5]:
    y1.append(i)
for i in lk_ball_wtd[0:5]:
    y1.append(i)
for i in cav[0:5]:
    y1.append(i)
for i in buns_bb_heavy[0:5]:
    y1.append(i)
for i in hbond_sr_bb[0:5]:
    y1.append(i)
for i in acc[0:5]:
    y1.append(i)
for i in p_aa_pp[0:5]:
    y1.append(i)
for i in packstat[0:5]:
    y1.append(i)
for i in dSASA_int[0:5]:
    y1.append(i)

for i in hbond_sc[5:10]:
    y2.append(i)
for i in ref[5:10]:
    y2.append(i)
for i in exphyd[5:10]:
    y2.append(i)
for i in fa_elec[5:10]:
    y2.append(i)
for i in hbonds_int[5:10]:
    y2.append(i)
for i in hbond_bb_sc[5:10]:
    y2.append(i)
for i in nres_int[5:10]:
    y2.append(i)
for i in hbond_lr_bb[5:10]:
    y2.append(i)
for i in fa_atr[5:10]:
    y2.append(i)
for i in lk_ball_wtd[5:10]:
    y2.append(i)
for i in cav[5:10]:
    y2.append(i)
for i in buns_bb_heavy[5:10]:
    y2.append(i)
for i in hbond_sr_bb[5:10]:
    y2.append(i)
for i in acc[5:10]:
    y2.append(i)
for i in p_aa_pp[5:10]:
    y2.append(i)
for i in packstat[5:10]:
    y2.append(i)
for i in dSASA_int[5:10]:
    y2.append(i)


for i in hbond_sc[10:15]:
    y3.append(i)
for i in ref[10:15]:
    y3.append(i)
for i in exphyd[10:15]:
    y3.append(i)
for i in fa_elec[10:15]:
    y3.append(i)
for i in hbonds_int[10:15]:
    y3.append(i)
for i in hbond_bb_sc[10:15]:
    y3.append(i)
for i in nres_int[10:15]:
    y3.append(i)
for i in hbond_lr_bb[10:15]:
    y3.append(i)
for i in fa_atr[10:15]:
    y3.append(i)
for i in lk_ball_wtd[10:15]:
    y3.append(i)
for i in cav[10:15]:
    y3.append(i)
for i in buns_bb_heavy[10:15]:
    y3.append(i)
for i in hbond_sr_bb[10:15]:
    y3.append(i)
for i in acc[10:15]:
    y3.append(i)
for i in p_aa_pp[10:15]:
    y3.append(i)
for i in packstat[10:15]:
    y3.append(i)
for i in dSASA_int[10:15]:
    y3.append(i)
...
ax.set_title('Distribution of Percent Difference Values')
xlabs=['hbond_sc','ref','exphyd','fa_elec','hbonds_int','hbond_bb_sc','nres_int',
       'hbond_lr_bb','fa_atr','lk_ball_wtd','cav','buns_bb_heavy','hbond_sr_bb','acc',
       'p_aa_pp','packstat','dSASA_int']
xlocs=[x+1 for x in range(len(xlabs))]
plt.xticks(xlocs, xlabs, rotation='vertical')
leglabs=['FD','BRE','CM']
plt.legend(leglabs,loc='upper right')
plt.tight_layout()
xf=[i+1 for i in range(len(xlabs))]
yf=[]
for i in range(17):
    yf.append(0)
ax.plot(xf,yf,linestyle=':')
plt.savefig('distsssss.pdf')
plt.show()

xlabs=['total_score','hbond_sc','ref','exphyd','fa_elec','hbonds_int','hbond_bb_sc','nres_int',
       'hbond_lr_bb','fa_atr','lk_ball_wtd','cav','buns_bb_heavy','hbond_sr_bb','acc',
       'p_aa_pp','packstat','dSASA_int']
df2=pd.DataFrame()
for i in xlabs:
    df2[i]=df[i]
corr_cos = df2.corr()
pd.DataFrame(corr_cos).to_csv("correlationsselect.csv")
